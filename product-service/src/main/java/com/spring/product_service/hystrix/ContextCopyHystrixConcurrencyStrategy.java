...
public class ContextCopyHystrixConcurrencyStrategy extends HystrixConcurrencyStrategy {
    ...
    public <T> Callable<T> wrapCallable(Callable<T> callable) {
        return new MyCallable(callable, ThreadLocalsHolder.getCorrelationId());
    }
    ...
    public static class MyCallable<T> implements Callable<T> {
        ...
        public T call() throws Exception {
            ThreadLocalsHolder.setCorrelationId(correlationId);
            try {
                return actual.call();
            } finally {
                ThreadLocalsHolder.setCorrelationId(null);
            }
        }
    }
}
